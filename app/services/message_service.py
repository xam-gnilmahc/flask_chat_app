...
class MessageService:
    ...
    @staticmethod
    def get_background_image(user_a: int, user_b: int) -> str:
        a, b = MessageService._normalize_pair(user_a, user_b)
        data = (
            get_supabase()
        ...
        if data.data and len(data.data) > 0:
            return data.data[0].get("background_image")
        return None

    @staticmethod
    def set_background_image(user_a: int, user_b: int, image_path: str) -> str:
        a, b = MessageService._normalize_pair(user_a, user_b)
        supabase = get_supabase()
        existing = (
            supabase.table("conversation_settings")
            .select("user_a")
            .eq("user_a", a)
            .eq("user_b", b)
            .execute()
        ...
        if existing.data and len(existing.data) > 0:
            supabase.table("conversation_settings").update(
                {"background_image": image_path}
            ).eq("user_a", a).eq("user_b", b).execute()
        else:
            supabase.table("conversation_settings").insert(
                {"user_a": a, "user_b": b, "background_image": image_path}
            ).execute()
        return image_path
