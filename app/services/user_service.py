...
class UserService:
    ...
    def get_by_username(username: str):
        data = (
            get_supabase().table("users").select("*").eq("username", username).execute()
        )
        return data.data[0] if data.data else None
    ...
    def update_profile_pic(user_id: int, filename: str):
        get_supabase().table("users").update({"profile_pic": filename}).eq(
            "id", user_id
        ).execute()
    ...
    def increment_token_version(user_id: int):
        ...
        get_supabase().table("users").update({"token_version": new_version}).eq(
            "id", user_id
        ).execute()
        return new_version
