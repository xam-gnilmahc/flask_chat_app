...
// ═══════════════════════════════════════════════════════════════════════════════
// EVENT: bg_image_changed — The other user changed the chat background
// ═══════════════════════════════════════════════════════════════════════════════
//
// WHAT HAPPENS:
//   Device A picks a new background image → emits "change_bg_image"
//   → Server relays "bg_image_changed" to Device B
//   → Device B's chat background updates in real-time.
//
// WHY NEEDED:
//   Both users in a chat should see the same background. When one changes it,
//   the other should see the change immediately without refreshing.
//
// WHAT THIS HANDLER DOES:
//   1. Checks if the event came from the user we're currently chatting with
//   2. If yes → updates the chat background to the new image
//   3. If image_url is empty → removes the background (reverts to default)
//
// ═══════════════════════════════════════════════════════════════════════════════
socket.on("bg_image_changed", (data) => {
  // Only update if this event is from the user we're currently chatting with
  if (data.from_user_id === activeUserId) {
    ...
    if (data.image_url) {
      ...
      // Remove the background (revert to solid color)
